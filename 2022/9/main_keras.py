...
model.add(Conv2D(filters=1 + L, kernel_size=3, input_shape=(N, N, 1 + L), use_bias=False, **common_kw))
...
for i in range(1, L):
    ...
    model.add(Conv2D(filters=1 + L + 9, kernel_size=3, activation='relu',
                     bias_initializer=tf.keras.initializers.Constant(-1), **common_kw))
    # This convolution layer collects the 9 different situations and update the knot into the new position.
    model.add(Conv2D(filters=1 + L, kernel_size=1, activation='relu', use_bias=False, **common_kw))
