...
def main():
    ...
    tf.print("Winner board: ", winner_board)
    tf.print("Last number: ", last_number)

    # Sum all unmarked numbers
    unmarked_sum = tf.reduce_sum(
        tf.gather_nd(winner_board, tf.where(tf.not_equal(winner_board, -1)))
    )
    tf.print("Unmarked sum: ", unmarked_sum)

    final_score = unmarked_sum * last_number
