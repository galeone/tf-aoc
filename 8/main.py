...
def main():
    ...
    decoded_output = tf.Variable(0, trainable=False, dtype=tf.int64)  # per display
    decoded_count = tf.Variable(0, trainable=False, dtype=tf.int32)
    ...
    for signal_patterns, output_digits in dataset:
        ...
        # Decode the output depending on the info we have in the input.
        # If we are able to decode starting from the outputs digits, it's better since this is already the result we are looking for.
        for num_segments, number in [(7, 8), (4, 4), (3, 7), (2, 1)]:
            _, digit_found, digit_positions = search_by_segments(
                output_digits, num_segments
            )
            if tf.greater(digit_found, 0):
                decoded_count.assign_add(digit_found)
                decoded_output.assign_add(tf.reduce_sum(number * 10 ** digit_positions))

        if False:
            count.assign_add(decoded_output)
            decoded_output.assign(0)
            tf.print("swat")
            sys.exit(1)
        else:
            zero_chars = [""]
